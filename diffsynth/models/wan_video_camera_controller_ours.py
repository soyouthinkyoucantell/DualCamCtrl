...
def adjust_intrinsics_after_resize_crop(
    K_origin, orig_size, zoom_size, crop_size, crop_type="center"
):
    """
    Args:
        K_origin: tuple of floats (fx, fy, cx, cy), all normalized (relative to orig_size)
        orig_size: (W_orig, H_orig)
        zoom_size: (W_zoom, H_zoom)
        crop_size: (W_crop, H_crop)
        crop_type: 'center' or 'topleft'

    Returns:
        K_new: (fx_new, fy_new, cx_new, cy_new), normalized w.r.t. crop_size
    """
    fx, fy, cx, cy = K_origin
    W_orig, H_orig = orig_size
    W_zoom, H_zoom = zoom_size
    ...
    # Step 2: 裁剪偏移
    if crop_type == "center":
        x_offset = (W_zoom - W_crop) / 2
        y_offset = (H_zoom - H_crop) / 2
    elif crop_type == "topleft":
        x_offset = 0
        y_offset = 0
    else:
    ...
    # Step 3: 裁剪后的主点位置（像素）
    cx_crop = cx_zoom - x_offset
    cy_crop = cy_zoom - y_offset

    # Step 4: 归一化到裁剪后图像尺寸
    fx_new = fx_zoom / W_crop
    fy_new = fy_zoom / H_crop
    cx_new = cx_crop / W_crop
    cy_new = cy_crop / H_crop
...
def process_pose_file(
    ...
    if return_poses:
        ...
        # 设置默认 crop 尺寸（如果没有裁剪，默认等于缩放后的尺寸）
        crop_width = crop_width or width
        crop_height = crop_height or height

        orig_size = (original_pose_width, original_pose_height)
        zoom_size = (width, height)
        crop_size = (crop_width, crop_height)

        intrinsic = []
        for cam_param in cam_params:
            # 使用归一化内参计算裁剪后新归一化内参
            K_origin = (cam_param.fx, cam_param.fy, cam_param.cx, cam_param.cy)
            fx_new, fy_new, cx_new, cy_new = adjust_intrinsics_after_resize_crop(
                K_origin, orig_size, zoom_size, crop_size, crop_type="center"
            )
            intrinsic.append(
                [
